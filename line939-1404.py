    ...
    def check_inv():
        def view():
            try:
                ...
                for product in products:
                    TEXT_PRODUCT.insert('end',
                                        f'Seller ID: {product[0]}, Product ID: {product[1]}, Product Name: {product[2]}, Stock: {product[3]}\n')
...
def buyer_window(ID, Address, Phone, Name, email, buyer_win, server):
    def CHECK_OUT(CART_ID, QUANTITY, COST, NAME, CCART):
        ...
        def PAY():
            print("PAY button pressed")  # <-- Debug: check if the function is called
            mode_payment = payment.get()
            print(f"Selected payment method: '{mode_payment}'")
            ...
            print(f"Saving to DB: Payment Method = {mode_payment}")
            ...
            try:
                ...
                if int(QUANTITY) > int(check_quantity):
                    messagebox.showwarning(title="WARNING",message="NO MORE AVAILABLE stock in the market pls. remove it from your cart or lessen your quantity")
                    return
                ...
                int(Bill_ID), mode_payment, payment_status, int(COST)))
                cursor.commit()
        ...
        payment.set(E_PAYMENT['values'][0])

        print(f"Initial combo box value: '{payment.get()}'")
