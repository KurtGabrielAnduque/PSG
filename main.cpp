...
void removeZeroItems(const string& fileName) {
    ...
    outFile.close();
    cout<<endl;
    cout << "Items with 0 quantity have been removed!" << endl;
    cout<<endl;
}
...
int out;
        ...
        if (!username.empty()) {
			...
			clearscreen();
			   break;
            // add price
            // determine if the items are already shippped out
            // if na notif reremove in the list if the seller shipped out the order
